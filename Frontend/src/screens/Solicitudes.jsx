...
import { View, Text, StyleSheet, ScrollView, useWindowDimensions, ActivityIndicator, TouchableOpacity, Modal } from 'react-native';
  ...
  const handleChangeState = async (request, newState) => {
    ...
    try {
      ...
      setRequests((prev) => prev.map((r) =>
        r.idRequest === request.idRequest ? { ...r, state: newState } : r
      ));
    } catch {}
    finally {
      setSavingId(null);
        ...
        <View style={styles.contentFrame}>
          ...
          >
            ...
            <View style={styles.headerBlock}>
              <Text style={styles.pageTitle}>Solicitudes de impresión</Text>
            ...
            <View style={styles.table}>
              <Text style={styles.tableTitle}>{requests.length} solicitud{requests.length !== 1 ? 'es' : ''}</Text>
              {loading ? (
                ...
                requests.slice().reverse().map((r) => {
                  const stateColors = STATE_COLORS[r.state?.toLowerCase()] || { bg: '#F3F4F6', text: '#6B7280' };
                  const user = usersMap[r.idUser];
                  const uName = user?.fullName || user?.full_name || `Usuario #${r.idUser}`;
                  const uDoc  = user?.document  ? `${user.typeDocument || 'Doc'}: ${user.document}` : '';
                    ...
                    <View key={r.idRequest} style={styles.requestCard}>
                      <View style={styles.requestCardTop}>
                        <View style={styles.requestInfo}>
                          ...
                          <Text style={styles.requestMeta}>{r.requestTipe} · {r.cardTipe}</Text>
                          {r.registrationDate ? (
                            <Text style={styles.requestDate}>
                              {new Date(r.registrationDate).toLocaleDateString('es-CO', { day: '2-digit', month: 'short', year: 'numeric' })}
                            </Text>
                          ) : null}
                        </View>
                        <TouchableOpacity
                          ...
                          <View style={styles.stateBtns}>
                            {STATES.map((s) => {
                              ...
                              return (
                                <TouchableOpacity
                                  key={s}
                                  style={[styles.stateBtn, { backgroundColor: sc.bg, borderColor: sc.text }, isActive && styles.stateBtnActive]}
...
const styles = StyleSheet.create({
  ...
  table:         { backgroundColor: '#FFFFFF', marginTop: 15, padding: 6, borderRadius: 12, borderWidth: 1, borderColor: '#DDF7EC', gap: 10 },
